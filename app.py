...
def get_isbn_display(book_id, isbn, apply_isbn):
    # Common container for left alignment
    container_start = "<div style='display: flex; flex-direction: column; align-items: flex-start; justify-content: center; width: 100%;'>"
    container_end = "</div>"
    
    if has_open_author_position(conn, book_id):
        return f"{container_start}<span style='color:#4b5563; background-color:#f3f4f6; font-size:12px; font-weight:600; padding:2px 10px; border-radius:6px; border:1px solid #e5e7eb;'>Not Applicable</span>{container_end}"
    
    status_html = ""
    # Email status logic
    if pd.notna(isbn):
        with conn.session as s:
            # Check if any author's isbn_sent_at is null
            authors_query = text("SELECT isbn_sent_at FROM book_authors WHERE book_id = :book_id")
            authors_res = s.execute(authors_query, {"book_id": book_id}).fetchall()
            
            if authors_res:
                all_sent = all(row[0] is not None for row in authors_res)
                if all_sent:
                    status_html = "<div style='color:#059669; font-size:9px; font-weight:700; margin-top:5px; text-transform:uppercase; letter-spacing:0.5px; display:flex; align-items:center; gap:3px;'><span>‎ ‎ ✓</span> SENT TO ALL</div>"
                else:
                    status_html = "<div style='color:#d97706; font-size:9px; font-weight:700; margin-top:5px; text-transform:uppercase; letter-spacing:0.5px; display:flex; align-items:center; gap:3px;'><span>‎ ‎ ○</span> ISBN EMAIL PENDING</div>"

    if pd.notna(isbn):
        return f"{container_start}<span style='color:#15803d; background-color:#ecfdf5; font-size:12px; font-weight:600; padding:2px 10px; border-radius:6px; border:1px solid #d1fae5;'>{isbn}</span>{status_html}{container_end}"
    elif apply_isbn == 0:
        return f"{container_start}<span style='color:#dc2626; background-color:#fef2f2; font-size:12px; font-weight:600; padding:2px 10px; border-radius:6px; border:1px solid #fee2e2;'>Not Applied</span>{container_end}"
    elif apply_isbn == 1:
        return f"{container_start}<span style='color:#1e40af; background-color:#eff6ff; font-size:12px; font-weight:600; padding:2px 10px; border-radius:6px; border:1px solid #dbeafe;'>Not Received</span>{container_end}"
    return f"{container_start}<span style='color:#1f2937; background-color:#f3f4f6; font-size:12px; font-weight:600; padding:2px 10px; border-radius:6px; border:1px solid #e5e7eb;'>-</span>{container_end}"
...
def pending_checklist_dialog(conn):
    ...
    # 1. Welcome mail (Calculate from registration date)
    welcome_mail_query = """
    SELECT b.book_id, b.title, b.date AS pending_since, ba.id as ba_id, ba.author_id, a.name AS author_name, ba.author_position
    FROM books b
    JOIN book_authors ba ON b.book_id = ba.book_id
    JOIN authors a ON ba.author_id = a.author_id
    WHERE ba.welcome_mail_sent = 0 AND b.is_cancelled = 0 AND b.deliver = 0 AND b.is_archived = 0
    ...
    WHERE b.cover_page_complete = 1 AND ba.cover_agreement_sent = 0
      AND b.is_cancelled = 0 AND b.deliver = 0 AND b.is_archived = 0
    ORDER BY pending_since ASC
    """
    cover_ready_data = conn.query(cover_ready_query, show_spinner=False, ttl=0)

    # 3. Operations complete (Calculate from latest operation end date)
    ops_complete_query = """
    SELECT b.book_id, b.title, 
           GREATEST(
               IFNULL(b.writing_end, '1000-01-01'), 
               IFNULL(b.proofreading_end, '1000-01-01'), 
               IFNULL(b.formatting_end, '1000-01-01'), 
               IFNULL(b.cover_end, '1000-01-01')
           ) AS pending_since,
           ba.id as ba_id, ba.author_id, a.name AS author_name, ba.author_position,
           b.correction_status,
           (SELECT COUNT(*) FROM corrections WHERE book_id = b.book_id AND correction_end IS NULL) as active_corrections
    FROM books b
    JOIN book_authors ba ON b.book_id = ba.book_id
    ...
    def render_checklist(df, field, key_prefix, log_label, description_text):
        ...
        with st.container(height=400, border=True):
            for book_id, group in df.groupby('book_id', sort=False):
                row = group.iloc[0]
                title = row['title']
                
                # Calculate days from the 'pending_since' column
                pending_date = pd.to_datetime(row['pending_since'])
                days_pending = (datetime.now().date() - pending_date.date()).days if pd.notnull(pending_date) else 0
                
                # Visual cues for urgency
                urgency_color = "#3b82f6" # Default blue
                urgency_label = ""
                ...
                st.markdown(f"""<div style='padding: 6px 10px; border-radius: 8px; border-left: 4px solid {urgency_color}; background-color: {urgency_color}08; margin-top: 8px; margin-bottom: 4px; display: flex; align-items: center; justify-content: space-between;'><div style='display: flex; align-items: center; gap: 8px;'><div style='font-size: 0.8em; font-weight: bold; color: #64748b;'>{book_id}</div><div style='font-size: 0.95em; font-weight: 600; color: #1e293b;'>{title}</div>{"<span style='color: #ef4444; font-size: 0.75em; font-weight: bold; background: #fee2e2; padding: 1px 5px; border-radius: 4px;'>⚠️ IN CORRECTION</span>" if is_in_correction else ""}</div>{urgency_label}</div>""", unsafe_allow_html=True)
                
                # List Authors for this book
                for _, author in group.iterrows():
                    col1, col2 = st.columns([0.85, 0.15])
                    with col1:
                        st.markdown(f"""<div style='padding-left: 10px; margin-bottom: 3px;'><span style='font-size: 0.9em; font-weight: 500;'>{author['author_name']}</span><span style='color: #94a3b8; font-size: 0.8em; margin-left: 6px;'>({author['author_position']})</span></div>""", unsafe_allow_html=True)
                    with col2:
...
def send_isbn_email(to_email, author_name, book_title, book_id, isbn, publisher="AGPH"):
    """
    Sends a formatted HTML email with the received ISBN.
    """
    if not to_email or not isbn:
        return False, None
    ...
    try:
        ...
        msg['From'] = email_address
        msg['To'] = to_email
        msg['Subject'] = f"ISBN Received - {book_title} | {pub_name}"
          ...
          <head>
            ...
            <style>
              @media only screen and (max-width: 600px) {{
                ...
                .isbn-box {{ font-size: 20px !important; padding: 15px !important; }}
            ...
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
              <tr>
                <td align="center" style="padding: 20px 0;">
                  <table class="container" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                    <!-- Top Accent Bar -->
                    <tr><td height="4" style="background-color: #1e4976;"></td></tr>
                    
                    <tr>
                      <td style="padding: 40px 30px;">
                        <h1 class="header-text" style="margin: 0 0 10px 0; color: #111827; font-size: 24px; font-weight: 700; letter-spacing: -0.025em;">ISBN Allocated</h1>
                        <p style="margin: 0; color: #6b7280; font-size: 14px; text-transform: uppercase; letter-spacing: 0.05em;">{pub_name}</p>
                        
                        <div style="margin: 30px 0; border-top: 1px solid #f3f4f6;"></div>
                        
                        <p class="body-text" style="margin: 0 0 20px 0; color: #374151; font-size: 16px; line-height: 1.6;">Dear <strong>{author_name}</strong>,</p>
                        <p class="body-text" style="margin: 0 0 30px 0; color: #374151; font-size: 16px; line-height: 1.6;">The International Standard Book Number (ISBN) for your upcoming work "<strong>{book_title}</strong>" has been officially registered.</p>
                        ...
                        <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 25px; text-align: center; margin-bottom: 30px;">
                          <span style="display: block; color: #64748b; font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 8px;">Registered ISBN</span>
                          <div class="isbn-box" style="color: #1e4976; font-size: 26px; font-weight: 800; letter-spacing: 1px; font-family: 'Courier New', Courier, monospace;">{isbn}</div>
                        </div>

                        <p class="body-text" style="margin: 0 0 30px 0; color: #4b5563; font-size: 15px; line-height: 1.6;">This unique identifier is now linked to your publication and will be used for cataloging and distribution across all platforms.</p>
                        
                        <div style="background-color: #f0f9ff; border-radius: 6px; padding: 15px; margin-bottom: 30px;">
                          <p style="margin: 0; color: #0369a1; font-size: 13px; line-height: 1.5;"><strong>Note:</strong> No further action is required from your side regarding ISBN allocation. Our team is proceeding with the next stages of publication.</p>
                        </div>

                        <p class="body-text" style="margin: 0; color: #374151; font-size: 16px;">Best regards,</p>
                        <p class="body-text" style="margin: 5px 0 0 0; color: #1e4976; font-size: 16px; font-weight: 600;">Team {pub_name}</p>
                      </td>
                    </tr>
                        ...
                        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-top: 25px;">
                          <tr>
                            <td style="color: #9ca3af; font-size: 12px; line-height: 1.5;">
                              Questions about your publication?<br>
                              Contact us at <a href="mailto:{pub_email}" style="color: #1e4976; text-decoration: none;">{pub_email}</a>
                            </td>
                          </tr>
                          <tr>
                            <td style="padding-top: 20px; color: #d1d5db; font-size: 11px;">
                              © 2026 {pub_name}. All rights reserved.
                            </td>
                          </tr>
                        </table>
                      </td>
            ...
            with connect_db().session as s_log:
                s_log.execute(
                    ...
                    """),
                    {
                        ...
                        "details": f"ISBN Email sent successfully with ISBN: {isbn}"
                    }
                )
                s_log.commit()
        except Exception:
            ...
            with connect_db().session as s_log:
                s_log.execute(
                    ...
                    """),
                    {
                        "timestamp": datetime.now(ist),
                        "recipient": to_email,
                        "sender": email_address if 'email_address' in locals() else "N/A",
                        "status": "Failed",
                        "book_id": book_id,
                        "title": book_title,
                        "user": st.session_state.get("username", "System"),
    ...
    with dialog_col2:    
        ...
        with st.expander("Authors", expanded=True):
            ...
            if authors_data.empty:
                ...
                with st.container(border=False):
                    col_h1, col_h2, col_h3 = st.columns(au_columns)
                    with col_h1:
                        st.markdown("#### Author Name")
                    with col_h2:
                        st.markdown("#### Position")
                    with col_h3:
                         st.markdown("#### Email ISBN")
                
                selected_authors_to_email = []
                # Determine current ISBN to show if it can be sent
                isbn_to_send = new_isbn if (apply_isbn and receive_isbn and new_isbn) else current_isbn
                can_send = bool(isbn_to_send)
                
                for _, author in authors_data.iterrows():
                    col_a1, col_a2, col_a3 = st.columns(au_columns)
                    with col_a1:
                    ...
                    with col_a3:
                        is_sent = pd.notnull(author['isbn_sent_at'])
                        if is_sent:
        ...
        with st.spinner("Saving changes..."):
            with conn.session as s:
                try:
                    ...
                    # Send ISBN emails if any authors selected
                    if selected_authors_to_email and isbn_to_send:
                        for author in selected_authors_to_email:
                            success, result = send_isbn_email(
                                author['email'], 
                                author['name'], 
                                new_title, 
                                book_id, 
                            ...
                            if success:
                                ...
                                with conn.session as s_email:
                                    s_email.execute(
                                        text("UPDATE book_authors SET isbn_sent_at = :timestamp WHERE id = :id"),
                                        {"timestamp": datetime.now(ist), "id": author['id']}
                                    )
                                    s_email.commit()
                                ...
                                log_activity(
                                    ...
                                    st.session_state.session_id,
                                    "sent isbn email",
                                    f"ISBN: {isbn_to_send}, Recipient: {author['name']} ({author['email']}), Book: {book_id}"
                                )
                                
                                st.toast(f"ISBN Email sent to {author['name']}!", icon="📧")
                            else:
                                st.error(f"Failed to send ISBN Email to {author['name']}: {result}")
...
def manage_price_dialog(book_id, conn):
    ...
    if book_authors.empty:
        ...
        # --- Grouping Logic ---
        # 1. Normalize corresponding_agent
        book_authors['agent_clean'] = book_authors['corresponding_agent'].fillna('').str.strip()
        
        # 2. Identify who is an agent for someone else
        all_agents = set(book_authors[book_authors['agent_clean'] != '']['agent_clean'])
        
        # 3. Assign groups
        # If an author has an agent, they belong to that agent's group.
        # If an author has no agent but IS an agent for others, they belong to their own name's group.
        # Otherwise, they are individuals (unique group ID).
        def assign_group(row):
            if row['agent_clean'] != '':
                return row['agent_clean']
            if row['name'] in all_agents:
                return row['name']
            return f"INDV_{row['id']}"
            
        book_authors['payment_group'] = book_authors.apply(assign_group, axis=1)
        
        # Aggregate summary for cards
        ...
        if total_book_paid >= total_book_expected and total_book_expected > 0:
            book_status_class = "status-paid"
            book_badge_text = "Fully Paid"
        elif total_book_paid > 0:
            book_status_class = "status-partial"
            book_badge_text = "Partially Paid"
        else:
            book_status_class = "status-pending"
            book_badge_text = "Pending"
        ...
        for i, row in agent_summary.iterrows():
            ...
            if amount_paid >= total_amount and total_amount > 0:
                ...
                status_text = f"₹{int(amount_paid)}/₹{total_amount}"
                badge_text = "Paid"
            elif amount_paid > 0:
    ...
    with st.container(border=True):
        ...
        if not book_authors.empty:
            ...
            for name, group_df in groups:
                if name.startswith("INDV_"):
                    tab_names.append(group_df.iloc[0]['name'])
                else:
                    tab_names.append(f"👤 {name} (Agent)")
            ...
            for tab, group_id in zip(tabs, group_keys):
                ...
                with tab:
                    # Lead author for anchoring payments
                    if group_id.startswith("INDV_"):
                        lead_author = group_df.iloc[0]
                        is_group = False
                    else:
                        # Prefer author whose name matches the agent name, else first in group
                        matches = group_df[group_df['name'] == group_id]
                        lead_author = matches.iloc[0] if not matches.empty else group_df.iloc[0]
                        is_group = True
                    ...
                    if not history.empty:
                        for _, p in history.iterrows():
                            # Find which author this payment was recorded under
                    ...
                    with acol1:
                        add_amt = st.text_input("Amount (₹)", key=f"add_amt_ui_{group_id}", placeholder="0")
                    with acol2:
                        add_date = st.date_input("Date", value=datetime.now(), key=f"add_date_ui_{group_id}")
                    with acol3:
                        add_mode = st.selectbox("Mode", ["Cash", "UPI", "Bank Deposit"], key=f"add_mode_ui_{group_id}")
                    
                    add_txn = ""
                    if add_mode in ["UPI", "Bank Deposit"]:
                        add_txn = st.text_input("Transaction ID", key=f"add_txn_ui_{group_id}", placeholder="Ref No.")
                        ...
                        try:
                            ...
                            if amt <= 0:
                                ...
                                with conn.session as s:
                                    ...
                                    """), {
                                        "ba_id": anchor_ba_id, "amt": amt, "date": add_date, "mode": add_mode, "txn": add_txn, "remark": add_rem,
                                        "status": initial_status, "created_by": st.session_state.get("user_id"),
...
with srcol_pending:
    ...
    if is_button_allowed("pending_checklist_dialog"):
        ...
        w_p_q = "SELECT COUNT(DISTINCT b.book_id) AS count FROM books b JOIN book_authors ba ON b.book_id = ba.book_id WHERE ba.welcome_mail_sent = 0 AND b.is_cancelled = 0 AND b.deliver = 0 AND b.is_archived = 0"
        c_p_q = "SELECT COUNT(DISTINCT b.book_id) AS count FROM books b JOIN book_authors ba ON b.book_id = ba.book_id WHERE b.cover_page_complete = 1 AND ba.cover_agreement_sent = 0 AND b.is_cancelled = 0 AND b.deliver = 0 AND b.is_archived = 0"
        o_p_q = "SELECT COUNT(DISTINCT b.book_id) AS count FROM books b JOIN book_authors ba ON b.book_id = ba.book_id WHERE (b.is_publish_only = 1 OR b.is_thesis_to_book = 1 OR b.writing_complete = 1) AND b.proofreading_complete = 1 AND b.formatting_complete = 1 AND b.cover_page_complete = 1 AND ba.digital_book_sent = 0 AND b.is_cancelled = 0 AND b.deliver = 0 AND b.is_archived = 0"
